...
def parsePoint(line):
    ...
    list1 = []
    list1 = line.split(',')
    pos = LabeledPoint(list1[0],list1[1:])
    return pos

#parsedSamplePoints = parsePoint(samplePoints[0])
parsedSamplePoints = map(parsePoint,rawData.take(5))
...
def squaredError(label, prediction):
    ...
    """
    value = (label-prediction)*(label - prediction)
    return value
...
def calcRMSE(labelsAndPreds):
    ...
    arg1 = labelsAndPreds.map(lambda x: x[0])    
    arg1l = arg1.take(arg1.count())
    arg2 = labelsAndPreds.map(lambda x: x[1])
    arg2l = arg2.take(arg2.count())
    #print arg1.take(3)
    val = map(squaredError,arg1l,arg2l)
    res = sum(val)/len(val)
    res = res**(1.0/2)
    return res
...
labelsAndPreds = parsedValData.map(lambda x: (x.label,x.features.dot(weightsLR0)))
...
print parsedTrainData.take(1)[0]
...
def twoWayInteractions(lp):
    ...
    """
    a= list(itertools.product(lp.features,lp.features))
    b = (map(lambda (x,y): x*y, a))
    c= map(lambda x: x,lp.features)
    d= b+c
    pos= LabeledPoint(lp.label,d)
    return pos
